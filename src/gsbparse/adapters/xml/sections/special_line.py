"""XML adapter: parse a ``<Special_line>`` element into a ``SpecialLineSection``."""
...
def parse_special_line_section(element: ET.Element) -> SpecialLineSection:
    """Parse a ``<Special_line>`` XML element into a :class:`SpecialLineSection`.

    Args:
        element: The ``<Special_line>`` XML element.

    Returns:
        A fully populated :class:`SpecialLineSection`.
    """
    a = element.attrib
